...
export const Feed: React.FC = () => {
  ...
  const displayedPosts = useMemo(() => {
    ...
    if (activeTab === 'FOR_YOU' && sortByPopularity) {
       list.sort((a, b) => {
          const scoreA = parseMetric(a.likes) + parseMetric(a.retweets) * 2;
          const scoreB = parseMetric(b.likes) + parseMetric(b.retweets) * 2;
          ...
          <article key={post.id} className="px-4 py-3 border-b border-gray-200 hover:bg-gray-50/40 transition-colors cursor-pointer">
            ...
            <div className="flex gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                {/* Image Attachment */}
                {post.image && (
                  <div className="mt-3 rounded-2xl overflow-hidden border border-gray-200 group relative shadow-sm">
                     <img 
                      src={post.image} 
                      alt="Post attachment" 
                      onClick={(e) => {
                      ...
                      className="w-full h-auto max-h-[600px] object-cover hover:opacity-95 transition-opacity cursor-pointer"
                    />
                    <button 
